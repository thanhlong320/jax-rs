...
data-source add --name=mysqlLocalDS --driver-name=mysql --jndi-name=java:jboss/datasources/LocalDS --connection-url=jdbc:mysql://$MYSQL_URI/$MYSQL_DATABASE --user-name=root --password=$MYSQL_ROOT_PASSWORD --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
data-source add --name=mysqlDasDS --driver-name=mysql --jndi-name=java:jboss/datasources/DasDS --connection-url=jdbc:mysql://$MYSQL_DAS_URI/$MYSQL_DAS_DATABASE --user-name=root --password=$MYSQL_DAS_ROOT_PASSWORD --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
...
# Deploy the WAR
cp /opt/jboss/wildfly/customization/SampleProject-1.0-SNAPSHOT.war $JBOSS_HOME/$JBOSS_MODE/deployments/SampleProject-1.0-SNAPSHOT.war
